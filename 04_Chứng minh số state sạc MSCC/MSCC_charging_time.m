function compare_charging_methods()
    ...
    % Tính toán cho MSCC (1-10 stages)
    max_stages = 10;
    stage_counts = 1:max_stages;
    t_MSCC = zeros(size(stage_counts));
    
    for n = stage_counts
        I_pattern = generate_optimal_pattern(n, I_CC, I_min);
        t_MSCC(n) = calculate_MSCC_time(I_pattern, V_cutoff, V_initial, R_eq, C_eq);
    end
    ...
    fprintf('Phương pháp MSCC:\n');
    for n = stage_counts
        fprintf('  - %d stage: %.2f s (≈%.2f phút)\n', n, t_MSCC(n), t_MSCC(n)/60);
    end
