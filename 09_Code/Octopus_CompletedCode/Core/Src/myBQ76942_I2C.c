...
uint16_t cellActiveBalancing = 0x00;
uint16_t cellLVL = 0x00;
...
uint16_t minTemp = 0;
uint16_t maxTemp = 0;
uint16_t maxinternalTemp = 0;
uint16_t CBinterval = 0;
uint16_t mincellVcharge = 0;
uint16_t MinDeltaCharge = 0;
uint16_t StopDeltaCharge = 0;
uint16_t MinCellVRelax = 0;
uint16_t MinDeltaRelax = 0;
uint16_t StopDeltaRelax = 0;
uint16_t ReadCheck = 0x00;
uint16_t Alarmrawbit = 0x00;

uint16_t CBStatus1 = 0;
uint16_t CBStatus2 = 0;
uint16_t CBStatus3 = 0;
uint16_t FetControl= 0;
...
void BQ769x2_Init() {
	...
	BQ769x2_SetRegister(CellBalanceStopDeltaCharge, 5, 2);
...
uint16_t BQ769x2_ReadAlarmStatus() {
	...
	AlarmBits = RX_data[1]*256 + RX_data[0];
...
{
	DirectCommands(CB_ACTIVE_CELLS, 0x02, R);
	cellActiveBalancing = (RX_data[1]*256 + RX_data[0]);
	return;
}

void BQ769x2_ReadLVL()
// Reads no of cells undergoing balancing.
{
	DirectCommands(CB_SET_LVL, 0x00, R);
	cellLVL = (RX_data[1]*256 + RX_data[0]);
	return;
}

// ********************************* End of BQ769x2 Measurement Commands   *****************************************
void BQ769x2_ReadCheckBatteryStatus()
// Reads no of cells undergoing balancing.
{
	DirectCommands(BatteryStatus, 0x00, R);
	ReadCheck = (RX_data[1]*256 + RX_data[0]);
	return;
}

void BQ769x2_ReadDataStatus1(){ // When read, this returns the 16-bit time in seconds that balancing has been active
	Subcommands(CBSTATUS1, 0x00, R);
	CBStatus1 = RX_32Byte[0];
	return;
}

void BQ769x2_ReadDataStatus2(){ //  When read, this returns a block containing the 32-bit cumulative balancing times in seconds for each of cells 1–8.
	Subcommands(CBSTATUS2, 0x00, R);
	CBStatus2 = ((RX_32Byte[3]<<24) + (RX_32Byte[2]<<16) + (RX_32Byte[1]<<8) + RX_32Byte[0]);
	return;
}

void BQ769x2_ReadDataStatus3(){ // When read, this returns a block containing the 32-bit cumulative balancing times in seconds for each of cells 9–10
	Subcommands(CBSTATUS3, 0x00, R);
	CBStatus3 = ((RX_32Byte[3]<<24) + (RX_32Byte[2]<<16) + (RX_32Byte[1]<<8) + RX_32Byte[0]);
	return;
}

void BQ769x2_ReadAlarmRawStatus()
// Reads no of cells undergoing balancing.
{
	DirectCommands(AlarmRawStatus, 0x00, R);
	Alarmrawbit = (RX_data[1]*256 + RX_data[0]);
	return;
}
